...
int main(void) {
    ...
    assert(epoch_revised_calendar_rulepack_project_constraints(&draft_rulepack, &projection) == 1);
    assert(strcmp(projection.calendar_system, "revised-13-month") == 0);
    assert(strcmp(projection.anchor_method, "measured-average-first-spring-day") == 0);
    assert(strcmp(projection.anchor_source, "owner-physical-measurement-required") == 0);
    assert(projection.month_count == 13);
    assert(projection.days_per_month == 28);
    assert(projection.common_intercalary_day_count == 1);
    assert(projection.leap_intercalary_day_count == 2);
    assert(projection.structure_ready == 1);
    assert(projection.conversion_ready == 0);
    assert(projection.customer_safe == 1);
    assert(strstr(projection.conversion_gate_reason, "owner-approved physical spring anchor") != 0);
