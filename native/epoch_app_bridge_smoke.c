...
int main(void) {
    ...
    assert(strcmp(snapshot.revised_anchor_method, "measured-average-first-spring-day") == 0);
    assert(strcmp(snapshot.revised_anchor_source, "owner-physical-measurement-required") == 0);
    assert(strcmp(snapshot.revised_year_opening_policy, "year-opening day outside the 13 months") == 0);
    assert(strcmp(snapshot.revised_leap_day_policy, "leap day outside the 13 months at end of year") == 0);
    assert(strstr(snapshot.revised_intercalary_policy, "common years have 1 day outside months") != 0);
    assert(strstr(snapshot.revised_conversion_gate_reason, "owner-approved physical spring anchor") != 0);
    ...
    assert(snapshot.revised_common_intercalary_days == 1);
    assert(snapshot.revised_leap_intercalary_days == 2);
    assert(snapshot.revised_constraints_customer_safe == 1);
