...
function renderCustomerAccountHistory() {
  const summary = recordTools.summarizeCustomerAccountHistoryState(data);
  const histories = summary.histories || [];
  byId("customer-account-history").innerHTML = histories.length
    ? histories.map((history) => {
      ...
      return record(
        history.displayName,
        eventPreview || history.customerSafeSummary,
        [
          toneChip(history.violations.length ? "needs review" : "customer-safe", history.violations.length ? "blocked" : "complete"),
          chip(`${history.eventCount} events`),
          chip(`${history.receiptCount} receipts`),
          chip(history.visibility)
        ]
      );
    }).join("")
    : record("Account History", "No customer account history records have been created yet.", [chip("empty")]);
}
...
function renderMonitor(items) {
  ...
  const authPrototype = report.authPrototype || recordTools.summarizeAuthProviderPrototypeState(data);
  ...
  const summaryCards = [
    ...
    record("Sandbox Auth Prototype", `${authPrototype.payloadReady} of ${authPrototype.prototypeCount} prototypes payload-ready; ${authPrototype.noLiveAuth} no-live-auth, ${authPrototype.noTokenStorage} no-token-storage, ${authPrototype.rawAdminDenied} raw-admin denied, ${authPrototype.rawMonitorDenied} raw-monitor denied.`, [toneChip(authPrototype.status, authPrototype.status === "ready" ? "complete" : "blocked"), chip(`${authPrototype.payloadEntries} payload items`)]),
    ...
    record("Account History", `${accountHistory.historyCount} durable customer account histor${accountHistory.historyCount === 1 ? "y" : "ies"} with ${accountHistory.timelineEvents} linked timeline events and ${accountHistory.receiptLinked} receipt-linked record${accountHistory.receiptLinked === 1 ? "" : "s"}.`, [toneChip(accountHistory.status, accountHistory.status === "ready" ? "complete" : "blocked"), chip(`${accountHistory.customerVisibleEvents} customer-safe events`)]),
  ...
  const notificationPrototypeCards = notificationPrototype.prototypes.map((prototype) => record(
    prototype.title,
    `${prototype.targetProvider} | ${prototype.adapterMode} | ${prototype.prototypeStatus} | ${prototype.payloadEntryCount} local preview items | ${prototype.notes}`,
    [
      statusChip(prototype.status),
  ...
  const authPrototypeCards = authPrototype.prototypes.map((prototype) => record(
    prototype.title,
    `${prototype.targetProvider} | ${prototype.adapterMode} | ${prototype.prototypeStatus} | ${prototype.payloadEntryCount} local preview items | ${prototype.notes}`,
    [
      statusChip(prototype.status),
  ...
  const accountHistoryCards = accountHistory.histories.map((history) => {
    const eventPreview = (history.statusTimeline || [])
      .slice(0, 3)
      .map((event) => `${event.sourceKind}: ${event.title}`)
      .join(" | ");
    return record(
      history.displayName,
      `${history.customerSafeSummary} | ${eventPreview || "Timeline pending"}`,
      [
        statusChip(history.status),
        chip(`${history.eventCount} events`),
        chip(`${history.receiptCount} receipts`),
        toneChip(`${history.violations.length} violations`, history.violations.length ? "blocked" : "complete")
      ]
    );
  });
  ...
  byId("monitor-items").innerHTML = [
    ...
    monitorSection("Sandbox Auth Provider", authPrototypeCards.length ? authPrototypeCards : [record("Sandbox Auth Provider", "No sandbox auth provider prototype records have been created yet.", [chip("empty")])], "monitor-auth-prototype"),
...
function renderNotificationPrototypeOptions() {
  const select = byId("notification-prototype-select");
  const submit = byId("notification-prototype-apply");
  if (!select) return;
  const prototypeOptions = (data.notificationProviderPrototypes || []).map((prototype) => {
    return `<option value="${escapeHtml(prototype.id)}">${escapeHtml(prototype.title)} (${escapeHtml(prototype.status)})</option>`;
  });
...
function renderAuthPrototypeOptions() {
  const select = byId("auth-prototype-select");
  const submit = byId("auth-prototype-apply");
  if (!select) return;
  const prototypeOptions = (data.authProviderPrototypes || []).map((prototype) => {
    return `<option value="${escapeHtml(prototype.id)}">${escapeHtml(prototype.title)} (${escapeHtml(prototype.status)})</option>`;
  });
  select.innerHTML = prototypeOptions.length ? prototypeOptions.join("") : `<option value="">No sandbox auth provider prototypes yet</option>`;
  if (submit) submit.disabled = !prototypeOptions.length;
}
...
function wireNotificationPrototypeForm() {
  const form = byId("notification-prototype-form");
  const confirmation = byId("notification-prototype-confirmation");
  if (!form || !confirmation) return;
  ...
  form.addEventListener("submit", (event) => {
    ...
    const payload = Object.fromEntries(formData.entries());
    try {
      const result = recordTools.transitionNotificationProviderPrototypeRecords(data, payload, {
        now: "2026-06-01T18:05:00.000Z"
      });
      data = result.data;
      persistData({
        adapterState: "modified-local",
        recoveryNote: "Sandbox notification provider prototype changed locally; export a ledger snapshot before live notification provider work."
      });
      renderAll();
      confirmation.innerHTML = record(
        "Sandbox Notification Provider Updated",
        `${result.records.prototype.title} is ${result.records.prototype.prototypeStatus}; live email, LINE, SMS, NEXUS, OAuth, secrets, webhooks, provider writes, and customer-visible sends remain disabled.`,
        [statusChip(result.records.prototype.status), chip(result.records.prototype.payloadMode), chip(result.records.prototype.sandboxOnly ? "sandbox-only" : "sandbox-missing")]
      );
      form.reset();
    } catch (error) {
      confirmation.innerHTML = record(
        "Sandbox Notification Provider Blocked",
        error.message || "Sandbox notification provider action could not be applied.",
        [chip("blocked", "blocked")]
      );
    }
  });
...
function wireAuthPrototypeForm() {
  const form = byId("auth-prototype-form");
  const confirmation = byId("auth-prototype-confirmation");
  if (!form || !confirmation) return;
  ...
  form.addEventListener("submit", (event) => {
    ...
    const payload = Object.fromEntries(formData.entries());
    try {
      const result = recordTools.transitionAuthProviderPrototypeRecords(data, payload, {
        now: "2026-06-01T18:50:00.000Z"
      });
      data = result.data;
      persistData({
        adapterState: "modified-local",
        recoveryNote: "Sandbox auth provider prototype changed locally; export a ledger snapshot before live identity-provider work."
      });
      renderAll();
      confirmation.innerHTML = record(
        "Sandbox Auth Provider Updated",
        `${result.records.prototype.title} is ${result.records.prototype.prototypeStatus}; production login, OAuth, secrets, credentials, token storage, refresh-token storage, webhooks, provider writes, external sessions, customer-visible auth behavior, and raw admin/monitor exposure remain disabled.`,
        [statusChip(result.records.prototype.status), chip(result.records.prototype.payloadMode), chip(result.records.prototype.sandboxOnly ? "sandbox-only" : "sandbox-missing")]
      );
      form.reset();
    } catch (error) {
      confirmation.innerHTML = record(
        "Sandbox Auth Provider Blocked",
        error.message || "Sandbox auth provider action could not be applied.",
        [chip("blocked", "blocked")]
      );
    }
  });
